...
export const DemoSwitcher: React.FC<DemoSwitcherProps> = ({ onRoleSwitched }) => {
      ...
      <div className="bg-slate-900 text-slate-100 border-b border-slate-800 px-3 py-2 text-xs flex flex-wrap items-center justify-between gap-2 sticky top-0 z-40 shadow-sm">
          ...
          {demoAccounts.map((acc) => {
            ...
            return (
              <button
                key={acc.key}
                onClick={() => {
                  store.setCurrentRole(acc.key as any);
                  onRoleSwitched?.();
                }}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded-md bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[11px] font-bold transition-colors"
