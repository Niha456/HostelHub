...
class HostelStore {
  ...
  public addAuditLog(action: string, target: string, details?: string) {
    const entry: AuditLogEntry = {
      ...
      details,
      actorName: this.currentUser?.name || 'System Administrator',
  ...
  public addNotification(
    recipientOrConfig:
      | string
      | { title: string; message: string; targetRole?: string; urgent?: boolean; type?: AppNotification['type']; recipientId?: string },
    title?: string,
    message?: string,
    type?: AppNotification['type']
  ) {
    if (typeof recipientOrConfig === 'object') {
      const notif: AppNotification = {
        id: 'NOTIF-' + Math.random().toString(36).substring(2, 8),
        recipientId: recipientOrConfig.recipientId || 'ALL',
        title: recipientOrConfig.title,
        message: recipientOrConfig.message,
        type: recipientOrConfig.type || (recipientOrConfig.urgent ? 'EMERGENCY' : 'ANNOUNCEMENT'),
        read: false,
        timestamp: 'Just now',
        targetRole: recipientOrConfig.targetRole,
      };
      this.notifications = [notif, ...this.notifications];
      this.addAuditLog('Broadcasted Notification', notif.title);
      this.notify();
      this.showToast('success', 'Notification broadcasted');
      return;
    }

    const notif: AppNotification = {
      id: 'NOTIF-' + Math.random().toString(36).substring(2, 8),
      recipientId: recipientOrConfig,
  ...
  public get accounts() {
    const accs = authService.getAccounts();
    return accs.map((a) => ({
      id: a.id,
      name: a.name,
      email: a.email,
  ...
  public toggleAccountStatus(id: string, newStatus?: boolean) {
    authService.toggleAccountStatus(id);
    this.addAuditLog('Toggled Account Status', `Account ID: ${id}`);
    this.notify();
    this.showToast('info', 'Account status updated');
  }

  public updateComplaintStatus(complaintId: string, status: ComplaintStatus) {
    const comp = this.complaints.find((c) => c.id === complaintId);
    if (!comp) return;
    comp.status = status;
    comp.updatedAt = new Date().toISOString();
    this.addAuditLog('Updated Complaint Status', `${complaintId} -> ${status}`);
    this.notify();
    this.showToast('info', `Complaint status updated to ${status}`);
  }

  public resolveComplaint(complaintId: string, resolutionNotes?: string, afterPhotoUrl?: string) {
    const comp = this.complaints.find((c) => c.id === complaintId);
    if (!comp) return;
    comp.status = 'RESOLVED';
    comp.resolvedAt = new Date().toISOString();
    if (resolutionNotes) comp.resolutionNotes = resolutionNotes;
    if (afterPhotoUrl) comp.afterPhotoUrl = afterPhotoUrl;
    comp.updatedAt = new Date().toISOString();

    const task = this.maintenanceTasks.find((t) => t.complaintId === complaintId);
    if (task) {
      task.status = 'COMPLETED';
      task.completedAt = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      if (resolutionNotes) task.resolutionNotes = resolutionNotes;
      if (afterPhotoUrl) task.afterPhoto = afterPhotoUrl;
    }

    this.addAuditLog('Resolved Complaint Ticket', `${complaintId}`, resolutionNotes);
    this.addNotification(
      comp.studentId,
      'Complaint Resolved!',
      `Your complaint ticket ${comp.id} (${comp.title}) has been verified and resolved.`,
      'COMPLAINT'
    );
    this.notify();
    this.showToast('success', `Complaint ${complaintId} marked resolved`);
  }

  public scanQRCode(codeString: string): { success: boolean; message: string; type?: string; data?: any } {
    if (!codeString || !codeString.trim()) {
      return { success: false, message: 'Empty or unreadable QR code payload.' };
    }

    const code = codeString.trim();

    // 1. Outpass QR Code (HOSTELHUB:OUT:<outpassId> or OUT-...)
    const outpassMatch = code.match(/OUT-[\w-]+/) || (code.includes('HOSTELHUB:OUT:') ? [code.split(':')[2]] : null);
    if (outpassMatch && outpassMatch[0]) {
      const opId = outpassMatch[0];
      const op = this.outpasses.find((o) => o.id === opId);
      if (!op) {
        return { success: false, message: `Outpass pass ${opId} was not found in active hostel records.` };
      }

      if (op.status === 'PENDING') {
        return { success: false, message: `Outpass ${op.id} has NOT been approved yet by the Warden.` };
      }

      if (op.status === 'REJECTED') {
        return { success: false, message: `Outpass ${op.id} was rejected by Warden. Access denied.` };
      }

      if (op.status === 'APPROVED') {
        this.recordOutpassExit(op.id);
        return {
          success: true,
          type: 'OUTPASS_EXIT',
          message: `Exit verified for ${op.studentName} (Room ${op.studentRoom}). Marked OUTSIDE campus.`,
          data: op,
        };
      }

      if (op.status === 'ACTIVE_OUT') {
        this.recordOutpassReturn(op.id);
        return {
          success: true,
          type: 'OUTPASS_RETURN',
          message: `Return recorded for ${op.studentName} (Room ${op.studentRoom}). Marked INSIDE campus.`,
          data: op,
        };
      }

      if (op.status === 'RETURNED') {
        return { success: false, message: `Outpass ${op.id} was already completed & closed.` };
      }
    }

    // 2. Visitor Pass QR Code (HOSTELHUB:VIS:<visitorId> or VIS-...)
    const visitorMatch = code.match(/VIS-[\w-]+/) || (code.includes('HOSTELHUB:VIS:') ? [code.split(':')[2]] : null);
    if (visitorMatch && visitorMatch[0]) {
      const visId = visitorMatch[0];
      const vis = this.visitors.find((v) => v.id === visId);
      if (!vis) {
        return { success: false, message: `Visitor permit ${visId} not found in hostel records.` };
      }

      if (vis.status === 'PENDING') {
        return { success: false, message: `Visitor permit for ${vis.visitorName} is still pending Warden approval.` };
      }

      if (vis.status === 'APPROVED') {
        this.checkInVisitor(vis.id);
        return {
          success: true,
          type: 'VISITOR_CHECKIN',
          message: `Visitor Check-In approved: ${vis.visitorName} (${vis.relationship}) visiting Room ${vis.studentRoom}.`,
          data: vis,
        };
      }

      if (vis.status === 'CHECKED_IN') {
        this.checkOutVisitor(vis.id);
        return {
          success: true,
          type: 'VISITOR_CHECKOUT',
          message: `Visitor Check-Out recorded: ${vis.visitorName} departure logged.`,
          data: vis,
        };
      }

      if (vis.status === 'CHECKED_OUT') {
        return { success: false, message: `Visitor permit for ${vis.visitorName} has already been used and closed.` };
      }
    }

    // 3. Student Resident ID Card (HOSTELHUB:STUDENT:<studentId>:...)
    if (code.includes('HOSTELHUB:STUDENT:') || code.startsWith('STU')) {
      const parts = code.split(':');
      const sId = parts.length > 2 ? parts[2] : code;
      const st = this.students.find((s) => s.id === sId || s.studentId === sId);
      if (st) {
        this.addAuditLog('Security Gate ID Verification', `${st.name} (${st.studentId}) Room ${st.room}`);
        return {
          success: true,
          type: 'STUDENT_RESIDENT_ID',
          message: `Verified Resident: ${st.name} (${st.studentId}) • Room ${st.room} (${st.block})`,
          data: st,
        };
      }
    }
    ...
    return {
      success: false,
      message: `Invalid or unrecognized QR barcode token: "${code}".`,
    };
  }

  public markAttendanceRecord(params: {
    studentId: string;
    date: string;
    ...
    const existing = this.attendanceRecords.find((a) => a.studentId === params.studentId && a.date === params.date);
    const student = this.students.find((s) => s.id === params.studentId || s.studentId === params.studentId);

    if (existing) {
      existing.status = params.status;
      if (params.remarks) existing.remarks = params.remarks;
      if (params.type) existing.type = params.type;
      existing.rollCallTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    } else {
      const newRec: AttendanceRecord = {
        id: 'att_' + Math.random().toString(36).substring(2, 9),
        studentId: params.studentId,
        studentName: student?.name || 'Resident Student',
        room: student?.room || 'A302',
        block: student?.block || 'Block A',
        date: params.date,
        status: params.status,
        rollCallTime: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        verifiedBy: params.markedBy || this.currentUser?.name || 'Night Warden',
        remarks: params.remarks,
        type: params.type,
      };
      this.attendanceRecords = [newRec, ...this.attendanceRecords];
    }
  ...
  public resetToSeedData() {
    ...
    this.students = fresh.students;
    this.complaints = fresh.complaints;
    this.maintenanceTasks = fresh.maintenanceTasks;
    this.visitors = fresh.visitors;
    this.outpasses = fresh.outpasses;
    this.emergencies = fresh.emergencies;
    this.announcements = fresh.announcements;
    this.notifications = fresh.notifications;
    this.attendanceRecords = fresh.attendanceRecords;
    this.auditLogs = fresh.auditLogs;
    this.saveToStorage();
    this.notify();
    this.showToast('success', 'HostelHub reset to clean seed data');
  }
}
